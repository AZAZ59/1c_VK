...
def preprocess_data(df) -> pd.DataFrame:
    if len(df.columns) != 44:
        ...
        df = df.iloc[5:]
        column_names = [
                'nm1', 'ps1', 'Unnamed: 2', 'key:', 'sz1', 'sz2', 'sz3', 'sz4', 'sz5', 'sz6', 'sz7', 'sz8', 'sz9',
                'sz10', 'sc',
                'Unnamed: 42', 'sp'
        ]
    ...
    for row in df.to_dict(orient='records'):
        ...
        if cur_state == State.Init and row['key:'] == 'key':
            ...
            if tryParse(cost):  # Начало
                ...
                cur_item.cost = cost
                for n in range(1, 11):
                    size = ''.join(c for c in str(row[f'sz{n}']) if c.isdigit())
                    if size is not None:
...
def write_to_excel(df, filename,_dir='./PriceFresh'):
    filename = filename.replace('/', '_')
    writer = pd.ExcelWriter( _dir + '/' + filename, engine='xlsxwriter')
...
if __name__ == '__main__':
    ...
    image_df = image_df.dropna(subset=['Фото'])
    ...
    write_to_excel(merged, 'Альбомы для ВК.xlsx', _dir = './File/' + date_XX_XX_XXXX + '/')
