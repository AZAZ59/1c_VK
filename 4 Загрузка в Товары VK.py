...
def main(group_name7):
    ...
    for group_name, df in df2.groupby('Группа'):
        ...
        for size in sizes:
            ...
            resp_json = get_request(action, hash=csrf_hash, group_id=group_id, lock_ver=lock_ver)
...
def upload_market_items(album_id, df, sizes_dict):
    for ind, row in tqdm(df.iterrows(), total=len(df)):
        ...
        if len(sizes) > 1:
            ...
            clone_resp = get_request(action, hash=csrf_hash, lock_ver=lock_ver)
        ...
        get_request(json.dumps(action_list), hash=csrf_hash, lock_ver=lock_ver)
